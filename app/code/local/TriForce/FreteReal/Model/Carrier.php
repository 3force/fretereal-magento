...
class TriForce_FreteReal_Model_Carrier extends Mage_Shipping_Model_Carrier_Abstract implements Mage_Shipping_Model_Carrier_Interface {
    ...
    public function collectRates(Mage_Shipping_Model_Rate_Request $request) {
        ...
        try {
            ...
            $fretes = explode(",", Mage::getStoreConfig('carriers/triforce_fretereal/shipping_methods'));
            if ($fretes == "") {
                return false;
            }
            ...
            foreach ($fretes as $freteKey => $freteCod) {
                $dadosParaAPI = array(
                    'client_id' => $client_id,
                ...
                if (isset($_SESSION['fretereal']) && $_SESSION['fretereal']['hash'][$freteCod] == $hash) {
                    ...
                    $_SESSION['fretereal'][$freteCod] = $ret;
                }
                ...
                if ($ret['status'] == 1) {
                    ...
                    $method->setCarrier("triforce_fretereal")
                                ...
                                ->setMethod($freteCod)
                                ->setMethodTitle(self::getTitulo($freteCod))
                                ->setCost($ret['fretes']['correios'][0]['valor'])
                                ->setPrice($ret['fretes']['correios'][0]['valor']);
